...
use SCFROAuth2\Controller\ResourceControllerInterface;
use SCFROAuth2\Controller\ResourceController;
use SCFROAuth2\OpenID\Controller\UserInfoControllerInterface;
use SCFROAuth2\OpenID\Controller\UserInfoController;
use SCFROAuth2\OpenID\Controller\AuthorizeController as OpenIDAuthorizeController;
use SCFROAuth2\OpenID\ResponseType\AuthorizationCode as OpenIDAuthorizationCodeResponseType;
use SCFROAuth2\OpenID\Storage\AuthorizationCodeInterface as OpenIDAuthorizationCodeInterface;
use SCFROAuth2\OpenID\GrantType\AuthorizationCode as OpenIDAuthorizationCodeGrantType;
use SCFROAuth2\Controller\AuthorizeControllerInterface;
use SCFROAuth2\Controller\AuthorizeController;
use SCFROAuth2\Controller\TokenControllerInterface;
use SCFROAuth2\Controller\TokenController;
use SCFROAuth2\ClientAssertionType\ClientAssertionTypeInterface;
use SCFROAuth2\ClientAssertionType\HttpBasic;
use SCFROAuth2\ResponseType\ResponseTypeInterface;
use SCFROAuth2\ResponseType\AuthorizationCode as AuthorizationCodeResponseType;
use SCFROAuth2\ResponseType\AccessToken;
use SCFROAuth2\ResponseType\JwtAccessToken;
use SCFROAuth2\OpenID\ResponseType\CodeIdToken;
use SCFROAuth2\OpenID\ResponseType\IdToken;
use SCFROAuth2\OpenID\ResponseType\IdTokenToken;
use SCFROAuth2\TokenType\TokenTypeInterface;
use SCFROAuth2\TokenType\Bearer;
use SCFROAuth2\GrantType\GrantTypeInterface;
use SCFROAuth2\GrantType\UserCredentials;
use SCFROAuth2\GrantType\ClientCredentials;
use SCFROAuth2\GrantType\RefreshToken;
use SCFROAuth2\GrantType\AuthorizationCode;
use SCFROAuth2\Storage\JwtAccessToken as JwtAccessTokenStorage;
use SCFROAuth2\Storage\JwtAccessTokenInterface;

/**
* Server class for OAuth2
* This class serves as a convience class which wraps the other Controller classes
*
* @see SCFROAuth2\Controller\ResourceController
* @see SCFROAuth2\Controller\AuthorizeController
* @see SCFROAuth2\Controller\TokenController
*/
    ...
    UserInfoControllerInterface
{
    ...
    protected $storageMap = array(
        'access_token' => 'SCFROAuth2\Storage\AccessTokenInterface',
        'authorization_code' => 'SCFROAuth2\Storage\AuthorizationCodeInterface',
        'client_credentials' => 'SCFROAuth2\Storage\ClientCredentialsInterface',
        'client' => 'SCFROAuth2\Storage\ClientInterface',
        'refresh_token' => 'SCFROAuth2\Storage\RefreshTokenInterface',
        'user_credentials' => 'SCFROAuth2\Storage\UserCredentialsInterface',
        'user_claims' => 'SCFROAuth2\OpenID\Storage\UserClaimsInterface',
        'public_key' => 'SCFROAuth2\Storage\PublicKeyInterface',
        'jwt_bearer' => 'SCFROAuth2\Storage\JWTBearerInterface',
        'scope' => 'SCFROAuth2\Storage\ScopeInterface',
    );

    protected $responseTypeMap = array(
        'token' => 'SCFROAuth2\ResponseType\AccessTokenInterface',
        'code' => 'SCFROAuth2\ResponseType\AuthorizationCodeInterface',
        'id_token' => 'SCFROAuth2\OpenID\ResponseType\IdTokenInterface',
        'id_token token' => 'SCFROAuth2\OpenID\ResponseType\IdTokenTokenInterface',
        'code id_token' => 'SCFROAuth2\OpenID\ResponseType\CodeIdTokenInterface',
    );

    /**
     * @param mixed                                                   $storage             (array or SCFROAuth2\Storage) - single object or array of objects implementing the
     ...
     * @param array                                                   $grantTypes          An array of SCFROAuth2\GrantType\GrantTypeInterface to use for granting access tokens
     * @param array                                                   $responseTypes       Response types to use.  array keys should be "code" and and "token" for
     *                                                                                     Access Token and Authorization Code response types
     * @param \SCFROAuth2\TokenType\TokenTypeInterface                     $tokenType           The token type object to use. Valid token types are "bearer" and "mac"
     * @param \SCFROAuth2\ScopeInterface                                   $scopeUtil           The scope utility class to use to validate scope
     * @param \SCFROAuth2\ClientAssertionType\ClientAssertionTypeInterface $clientAssertionType The method in which to verify the client identity.  Default is HttpBasic
     *
     ...
     *
     * @param $request - \SCFROAuth2\RequestInterface
     * Request object to grant access token
     *
     * @param $response - \SCFROAuth2\ResponseInterface
    ...
    public function addStorage($storage, $key = null)
    {
        ...
        if (isset($this->storageMap[$key])) {
            ...
            if ($key === 'client' && !isset($this->storages['client_credentials'])) {
                if ($storage instanceof \SCFROAuth2\Storage\ClientCredentialsInterface) {
                    $this->storages['client_credentials'] = $storage;
                }
            } elseif ($key === 'client_credentials' && !isset($this->storages['client'])) {
                if ($storage instanceof \SCFROAuth2\Storage\ClientInterface) {
    ...
    protected function createDefaultAuthorizeController()
    {
        if (!isset($this->storages['client'])) {
            throw new \LogicException('You must supply a storage object implementing \SCFROAuth2\Storage\ClientInterface to use the authorize server');
        }
    ...
    protected function createDefaultTokenController()
    {
            ...
            foreach ($this->grantTypes as $grantType) {
                if (!$grantType instanceof ClientAssertionTypeInterface) {
                    if (!isset($this->storages['client_credentials'])) {
                        throw new \LogicException('You must supply a storage object implementing SCFROAuth2\Storage\ClientCredentialsInterface to use the token server');
                    }
        ...
        if (!isset($this->storages['client'])) {
            throw new \LogicException('You must supply a storage object implementing SCFROAuth2\Storage\ClientInterface to use the token server');
        }
    ...
    protected function createDefaultResourceController()
    {
        if ($this->config['use_jwt_access_tokens']) {
            ...
            throw new \LogicException('You must supply a storage object implementing SCFROAuth2\Storage\AccessTokenInterface or use JwtAccessTokens to use the resource server');
        }
    ...
    protected function createDefaultUserInfoController()
    {
        if ($this->config['use_jwt_access_tokens']) {
            ...
            throw new \LogicException('You must supply a storage object implementing SCFROAuth2\Storage\AccessTokenInterface or use JwtAccessTokens to use the UserInfo server');
        }

        if (!isset($this->storages['user_claims'])) {
            throw new \LogicException('You must supply a storage object implementing SCFROAuth2\OpenID\Storage\UserClaimsInterface to use the UserInfo server');
        }
    ...
    protected function getDefaultResponseTypes()
    {
        ...
        if (isset($this->storages['authorization_code'])) {
            ...
            if ($this->config['use_openid_connect']) {
                if (!$this->storages['authorization_code'] instanceof OpenIDAuthorizationCodeInterface) {
                    throw new \LogicException('Your authorization_code storage must implement SCFROAuth2\OpenID\Storage\AuthorizationCodeInterface to work when "use_openid_connect" is true');
                }
        ...
        if (count($responseTypes) == 0) {
            throw new \LogicException('You must supply an array of response_types in the constructor or implement a SCFROAuth2\Storage\AuthorizationCodeInterface storage object or set "allow_implicit" to true and implement a SCFROAuth2\Storage\AccessTokenInterface storage object');
        }
    ...
    protected function createDefaultJwtAccessTokenStorage()
    {
        if (!isset($this->storages['public_key'])) {
            throw new \LogicException('You must supply a storage object implementing SCFROAuth2\Storage\PublicKeyInterface to use crypto tokens');
        }
        $tokenStorage = null;
    ...
    {
        if (!isset($this->storages['access_token'])) {
            throw new \LogicException('You must supply a response type implementing SCFROAuth2\ResponseType\AccessTokenInterface, or a storage object implementing SCFROAuth2\Storage\AccessTokenInterface to use the token server');
        }
    ...
    {
        if (!isset($this->storages['user_claims'])) {
            throw new \LogicException('You must supply a storage object implementing SCFROAuth2\OpenID\Storage\UserClaimsInterface to use openid connect');
        }
        if (!isset($this->storages['public_key'])) {
            throw new \LogicException('You must supply a storage object implementing SCFROAuth2\Storage\PublicKeyInterface to use openid connect');
        }
